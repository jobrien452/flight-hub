...
async def test_my_reports_spans_every_mission(
    client, pilot_headers, pilot_user: User, admin_user: User, assigned_mission: Mission
):
    second = Mission(
        name="Survey Site C",
    ...
    for mission in (assigned_mission, second):
    ...
    assert resp.status_code == 200
    assert {r["mission_id"] for r in resp.json()} == {str(assigned_mission.id), str(second.id)}


async def test_my_reports_does_not_leak_another_pilots_work(
    client, pilot_headers, other_pilot_headers, other_pilot_user: User, assigned_mission: Mission
):
    await client.post(f"/missions/{assigned_mission.id}/reports", json={}, headers=pilot_headers)
